...
SELECT           qit.id, qit.s_id, qit.ct_id, qit.c_id, qit.u_id, qit.inspector_id, qit.name, qit.year, qit.start_time, qit.end_time, qit.sort,qit.is_generate,     qit.remark, qit.create_time,s.id sid,s.name sname,s.sort ssort,ct.id ctid,ct.cc_id ctcc_id,ct.s_id cts_id,ct.name ctname,     ct.sort ctsort,ct.remark ctremark,c.id cid,c.name cname,c.sort csort,u.id uid,u.login_name ulogin_name,u.name uname,u2.id u2id ,u2.login_name u2login_name ,u2.name u2name     ,        qitd.id qitdid, qitd.qit_id qitdqit_id, qitd.check_project qitdcheck_project, qitd.check_point qitdcheck_point, qitd.frequency qitdfrequency,       qitd.qualified qitdqualified,qitd.score qitdscore, qitd.check_time qitdcheck_time, qitd.check_user qitdcheck_user, qitd.is_checked qitdis_checked,       qitd.task_stime qitdtask_stime, qitd.task_etime qitdtask_etime, qitd.sort qitdsort, qitd.enclosure qitdenclosure, qitd.remark qitdremark,       qitr.id qitrid, qitr.qitd_id qitrqitd_id, qitr.check_content qitrcheck_content, qitr.is_qualified qitris_qualified, qitr.score qitrscore,       qitr.check_time qitrcheck_time, qitr.check_situation qitrcheck_situation, qitr.enclosure qitrenclosure, qitr.remark qitrremark         FROM gw_quality_inspect_task qit     LEFT JOIN gw_supplier s      ON qit.s_id = s.id     LEFT JOIN gw_check_temp ct      ON qit.ct_id = ct.id     LEFT JOIN gw_company c      ON qit.c_id = c.id     LEFT JOIN sys_user u      ON qit.u_id = u.id     LEFT JOIN sys_user u2      ON qit.inspector_id = u2.id     LEFT JOIN gw_quality_inspect_task_detail qitd      ON qit.id = qitd.qit_id     LEFT JOIN gw_quality_inspect_task_result qitr      ON qitd.id = qitr.qitd_id     WHERE 1=1             AND qit.inspector_id = 38                 AND qit.end_time > ?  <AND  qit.start_time < ?
