...
public class SysRoleResourceServiceImpl implements SysRoleResourceService {
    ...
    @Override
    public List<SysRoleResourceForm> list2(SysRoleResourceForm form) {
        SysRoleResourceExample example = new SysRoleResourceExample();
        SysRoleResourceExample.Criteria criteria = example.createCriteria();
        criteria.andRoleIdEqualTo(form.getRoleId());
        List<SysRoleResource> sysRoleResourceList = sysRoleResourceMapper.selectByExample(example);
        return SysRoleResourceConvert.entityListToFormList(sysRoleResourceList);
    }
    ...
    @Override
    public BaseResult saveOrUpdate(long roleId ,List<SysRoleResourceForm> forms) {
        List<SysRoleResource> list = new ArrayList<SysRoleResource>();
        for (int i = 0; i < forms.size(); i++) {
            SysRoleResource entity = SysRoleResourceConvert.formToEntity(forms.get(i));
            list.add(entity);
        }
        SysRoleResourceExample sysRoleResourceExample = new SysRoleResourceExample();
        SysRoleResourceExample.Criteria criteria = sysRoleResourceExample.createCriteria();
        criteria.andRoleIdEqualTo(roleId);
        sysRoleResourceMapper.deleteByExample(sysRoleResourceExample);
        if (list.size()>0){
            sysRoleResourceMapper.saveList(list);
        }
        return BaseResult.ok("保存成功");
    }
